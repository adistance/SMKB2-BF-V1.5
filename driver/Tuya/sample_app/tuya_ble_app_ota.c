...
#define  DFU_SETTINGS_ADDR (2840)

#if 1
#define TUYA_OTA_PRINT_INFO0   APP_PRINT_TRACE0
#define TUYA_OTA_PRINT_INFO1   APP_PRINT_TRACE1
#define TUYA_OTA_PRINT_INFO2   APP_PRINT_TRACE2
#define TUYA_OTA_PRINT_INFO3   APP_PRINT_TRACE3
#define TUYA_OTA_PRINT_INFO4   APP_PRINT_TRACE4
#define TUYA_OTA_PRINT_INFO7   APP_PRINT_TRACE7
#else
...
extern uint8_t AesKeyBuf[32];
extern uint8_t AesNonceBuf[16];
uint8_t u8HeadData[256];

uint32_t dfu_flash_check_blank(uint16_t signature, uint32_t offset, uint16_t nSize);


uint32_t sil_dfu_update(uint16_t signature, uint32_t offset, uint32_t length, uint32_t/*void*/ *p_void);

static uint32_t s_u32UpgradeNum = 0;
...
static int16_t current_package = 0;
...
{
	TUYA_BLE_LOG_DEBUG("sizeof(dfu_settings_t) is %d", sizeof(dfu_settings_t));
...
uint32_t tuya_ota_init(void)
{
    ...
    TUYA_BLE_LOG_DEBUG("dfu_settings.progress.firmware_image_offset_last = %d ",dfu_settings.progress.firmware_image_offset_last);
    TUYA_BLE_LOG_DEBUG("dfu_settings.progress.firmware_image_crc_last = 0x%08x ",dfu_settings.progress.firmware_image_crc_last);
...
static void tuya_ota_file_info_req(uint8_t*recv_data,uint32_t recv_len)
{
    ...
    if(memcmp(dfu_settings.file_info.firmware_file_md5,&recv_data[17+TUYA_BLE_PRODUCT_ID_DEFAULT_LEN],16)==0)
    {
        file_md5 = true;
    }
    else
    ...
    TUYA_BLE_LOG_DEBUG("dfu_settings.file_info.firmware_file_version = 0x%04x ---- file_version = 0x%04x",dfu_settings.file_info.firmware_file_version,file_version);
    TUYA_BLE_LOG_DEBUG("dfu_settings.file_info.firmware_file_length = %d ---- file_length = %d",dfu_settings.file_info.firmware_file_length,file_length);
    TUYA_BLE_LOG_DEBUG("dfu_settings.file_info.firmware_file_crc = 0x%08x ---- file_crc = 0x%08x",dfu_settings.file_info.firmware_file_crc,file_crc);
    TUYA_BLE_LOG_DEBUG("dfu_settings.progress.firmware_image_offset_last = %d ",dfu_settings.progress.firmware_image_offset_last);
    TUYA_BLE_LOG_DEBUG("dfu_settings.progress.firmware_image_crc_last = 0x%08x ",dfu_settings.progress.firmware_image_crc_last);
    TUYA_BLE_LOG_DEBUG("file_md5 = %d",file_md5);

	//涂鸦每次传输519个字节，7个字节包头数据 + 512个字节的app升级数据
	s_u32UpgradeNum = file_length / 512 + (file_length % 512 > 0 ? 1 : 0);

	TUYA_BLE_LOG_DEBUG("s_u32UpgradeNum = %d", s_u32UpgradeNum);
		
    if((file_version > TY_APP_VER_NUM)&&(file_length <= APP_NEW_FW_MAX_SIZE))
    {

        if(file_md5&&(dfu_settings.file_info.firmware_file_version==file_version)&&(dfu_settings.file_info.firmware_file_length==file_length)
        ...
        else
        {
            ...
            dfu_settings.file_info.firmware_file_crc = file_crc;
            memcpy(dfu_settings.file_info.firmware_file_md5,&recv_data[17+TUYA_BLE_PRODUCT_ID_DEFAULT_LEN],16);
    ...
    else
    {
        ...
        {
            TUYA_OTA_PRINT_INFO0("ota file version error !");
            state = 2;
        }
        else
        {
            TUYA_OTA_PRINT_INFO0("ota file length is bigger than rev space !");
            state = 3;
        }
    }
    ...
    if(state==0)
    {
        ...
        if(dfu_settings.progress.firmware_image_offset_last>0)
        {
            ...
            TUYA_BLE_LOG_DEBUG("file_crc_check_in_flash-firmware_image_crc_last = 0x%08x ---- crc_temp = 0x%08x",dfu_settings.progress.firmware_image_crc_last,crc_temp);
  ...
  //  uint8_t encry_mode = 0;
    ...
    if(dfu_settings.progress.firmware_image_offset_last>0)
    {
        ...
        if(offset_temp)
        {
            ...
            if(dfu_settings.progress.firmware_image_offset_last>0)
            {
                file_crc_check_in_flash(dfu_settings.progress.firmware_image_offset_last,&dfu_settings.progress.firmware_image_crc_last);
            }
            else
            {
                dfu_settings.progress.firmware_image_crc_last = 0;
            }
            TUYA_BLE_LOG_DEBUG("tuya_ota_offset_req new firmware_image_offset_last : 0x%04x",dfu_settings.progress.firmware_image_offset_last);
            TUYA_BLE_LOG_DEBUG("tuya_ota_offset_req new firmware_image_crc_last    : 0x%04x",dfu_settings.progress.firmware_image_crc_last);
            dfu_settings.write_offset = dfu_settings.progress.firmware_image_offset_last;
            dfu_settings_write();
        }
    }
...
static uint32_t ota_flash_erase(uint32_t addr)
{
    uint32_t ret = 0;
    bool result = false;
...
int ota_write_flash(void)
{
	...
	id = (current_package + 1) / 8 - 1;
	p = (uint32_t *)g_ImgBuf;
	
	unlock_flash_bp_all();
	APP_PRINT_INFO1("-----upgrade_write_flash %d ------", id);	
	ota_flash_erase(earse_base+id * SECTOR_SIZE);
...
bool tuya_ota_cal_crc(void)
{	
	uint16_t u16CalCrc;
	uint16_t u16SumCrc;
	uint8_t u8SectorId = (current_package + 1) / 8 - 1;

	AesKeyBuf[0] =  0x31 + 1 + u8SectorId;			
	aes256_ctr(AesKeyBuf, AesNonceBuf, g_ImgBuf, SECTOR_SIZE);
	u16CalCrc = crc_ccitt(g_ImgBuf, SECTOR_SIZE);	
	u16SumCrc = (u8HeadData[36+1+u8SectorId*2] << 8) | (u8HeadData[36+u8SectorId*2]);

	TUYA_BLE_LOG_DEBUG("u16CalCrc is 0x%x , u16SumCrc is 0x%x, sectorId is %d",u16CalCrc,u16SumCrc, (current_package + 1) / 8 - 1);

	if(u16CalCrc != u16SumCrc)
		return false;
	else
		return true;
}
...
static void tuya_ota_data_req(uint8_t*recv_data,uint32_t recv_len)
{
	...
	TUYA_BLE_LOG_DEBUG("data_length is : %d， current_package is %d",data_length, current_package);
	APP_PRINT_INFO1("data is %b", TRACE_BINARY(data_length, &recv_data[7]));
	
	if(data_length>MAX_DFU_DATA_LEN)
    {
    ...
    {	
    	...
    	{
    		memset(u8HeadData, 0, sizeof(u8HeadData));
    		memcpy(u8HeadData, &recv_data[7], sizeof(u8HeadData)); 		
    	}
		
    	current_package--;
        if(current_package < s_u32UpgradeNum && (current_package >= 0))
        {
        	memcpy(g_ImgBuf+current_package%8*512, &recv_data[7], data_length);

			//每4k写一次数据
			if((current_package + 1) % 8 == 0 || (current_package + 1 == s_u32UpgradeNum))
			{
				if(tuya_ota_cal_crc())
				{
					ota_write_flash();
				...
				{
					TUYA_BLE_LOG_ERROR("ota received package data CRC error");
			        state = 4;
			        goto ota_data_response;
				}
				memset(g_ImgBuf, 0, 4096);
			...
			TUYA_BLE_LOG_DEBUG("current_package(%d) save success", current_package);	
        }
...
{
    TUYA_OTA_PRINT_INFO2("ota cmd : 0x%04x , recv_len : %d",cmd,recv_len);
	TUYA_OTA_PRINT_INFO1("OTA recv_data %b", TRACE_BINARY(recv_len, recv_data));
