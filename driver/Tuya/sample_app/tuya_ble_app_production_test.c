...
void tuya_ble_prod_monitor_timer_init(void)
{
...
void tuya_ble_prod_monitor_timer_start(void)
{
...
static  void tuya_ble_auc_write_auth_info(uint8_t *para, uint16_t len)
{
	...
	APP_PRINT_INFO1("key is %b", TRACE_BINARY(AUTH_KEY_LEN, &para[11]));
	APP_PRINT_INFO1("device id is %b", TRACE_BINARY(DEVICE_ID_LEN, &para[53]));
	APP_PRINT_INFO1("mac is %b", TRACE_BINARY(MAC_LEN, mac_temp));
	APP_PRINT_INFO1("mac_char is %b", TRACE_BINARY(MAC_LEN*2, mac_char));
	APP_PRINT_INFO1("pid is %b", TRACE_BINARY(pid_len, &para[pid_pos]));
	APP_PRINT_INFO0("****************************************************");
...
void tuya_ble_app_production_test_process(uint8_t channel,uint8_t *p_in_data,uint16_t in_len)
{
	...
	APP_PRINT_INFO1("tuya_ble_app_production_test_process cmd is %d", cmd);
   /* 
