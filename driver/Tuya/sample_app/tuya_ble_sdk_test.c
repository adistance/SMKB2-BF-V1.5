...
void tuya_ble_app_sdk_test_process(uint8_t channel, uint8_t *p_in_data, uint16_t in_len)
{
    ...
    switch (cmd->sub_id) {
        ...
        case TY_UARTV_CMD_GET_TIME: 
        {
            ...
            TUYA_APP_LOG_INFO("[%d-%d-%d %d:%d:%d weak=%d  timezone=%d]\r\n", rsp_data[0], rsp_data[1], rsp_data[2], rsp_data[3], rsp_data[4], rsp_data[5], rsp_data[6], test_param.timezone);
        }
            break;
        ...
        case TY_UARTV_CMD_GET_WEATHER:
        {
            ...
            rsp_data[rsp_len++] = 0;
            rsp_data[rsp_len++] = tuya_ble_feature_weather_data_request_with_location(cmd->value[0], combine_type, cmd->value[5]);
        }
            break;
		#endif 
...
void tuya_ble_sdk_test_get_time_rsp(tuya_ble_time_noraml_data_t* data)
{
    ...
    TUYA_APP_LOG_INFO("[%d-%d-%d %d:%d:%d weak=%d timezone=%d]\r\n", rsp_data[0], rsp_data[1], rsp_data[2], rsp_data[3], rsp_data[4], rsp_data[5], rsp_data[6], data->time_zone);
