...
static void tuya_ble_handle_dp_data_send_response(uint8_t*recv_data,uint16_t recv_len)
{
    ...
    event.dp_send_response_data.mode = (tuya_ble_dp_data_send_mode_t)recv_data[19];
...
static void tuya_ble_handle_dp_data_with_time_send_response(uint8_t*recv_data,uint16_t recv_len)
{
    ...
    event.dp_with_time_send_response_data.mode = (tuya_ble_dp_data_send_mode_t)recv_data[19];
...
{	
	APP_PRINT_INFO1("tuya_ble_evt_process cmd is 0x%x", cmd);
    switch(cmd)
    {
#if (TUYA_BLE_PROTOCOL_VERSION_HIGN==4) 
