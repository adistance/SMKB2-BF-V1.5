...
void tuya_ble_adv_change(void)
{
    ...
    else
    {
		...
		APP_PRINT_INFO1("adv_data before %b", TRACE_BINARY(sizeof(adv_data), adv_data));
		APP_PRINT_INFO1("scan_rsp_data before %b", TRACE_BINARY(sizeof(scan_rsp_data), scan_rsp_data));

		APP_PRINT_INFO1("encode before %b", TRACE_BINARY(DEVICE_ID_LEN, tuya_ble_current_para.auth_settings.device_id));

		APP_PRINT_INFO1("encode before %b", TRACE_BINARY(DEVICE_ID_LEN, tuya_ble_current_para.auth_settings.device_id));
		
        tuya_ble_device_id_encrypt_v4(&adv_data[11],adv_data[7]-3,tuya_ble_current_para.auth_settings.device_id,DEVICE_ID_LEN,encry_device_id);
		
		APP_PRINT_INFO1("encode after %b", TRACE_BINARY(DEVICE_ID_LEN, encry_device_id));
		
        memcpy(&scan_rsp_data[8],encry_device_id,DEVICE_ID_LEN);

		APP_PRINT_INFO1("adv_data after %b", TRACE_BINARY(sizeof(adv_data), adv_data));
		APP_PRINT_INFO1("scan_rsp_data after %b", TRACE_BINARY(sizeof(scan_rsp_data), scan_rsp_data));
    }
