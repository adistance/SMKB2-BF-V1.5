...
{
    USART_SendData((uint8_t *)p_data,len);
    return TUYA_BLE_SUCCESS;
}
...
{
	//APP_PRINT_INFO0("tuya_ble_aes128_ecb_encrypt-----");
    uint16_t length;
    if(input_len%16)
    {
        return false;
...
{
	//APP_PRINT_INFO0("tuya_ble_aes128_ecb_decrypt-----");
    uint16_t length;
    if(input_len%16)
    {
        return false;
...
{
	//APP_PRINT_INFO0("tuya_ble_aes128_cbc_encrypt-----");
    uint32_t IV[4] = {0};
    //uint8_t i = 0;
    uint32_t len; 
    if(input_len%16)
...
{
	//APP_PRINT_INFO0("tuya_ble_aes128_cbc_decrypt-----");
    uint32_t IV[4] = {0};
   // uint8_t i = 0;
    uint32_t len; 
