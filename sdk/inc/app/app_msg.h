...
typedef enum
{
    ...
    EVENT_APP_CUSTOM = 0x10, //tuya msg
} T_EVENT_TYPE;
...
typedef enum
{
    ...
    IO_MSG_TYPE_FINGERPRINT,    //menu任务中的指纹操作发过来的信息类型
    IO_MSG_TYPE_ENCRYPT_COMP,
...
/**  @brief IO subtype definitions for @ref IO_MSG_TYPE_LOCK_STEP_FIRST type */
typedef enum
{
    IO_MSG_FINGERPRINT_SUBTYPE_ENROLL       = 1, /**< local operation event */
    IO_MSG_FINGERPRINT_SUBTYPE_DELETE       = 2, /**< blue operation event*/
} T_IO_MSG_FINGERPRINT_SUBTYPE;
...
typedef struct
{
    ...
    uint16_t matchId;
    uint16_t score;
}T_IO_MSG_MATCH, *PT_IO_MSG_MATCH;

typedef struct
{
    uint8_t  enrollStep;
    uint8_t  progress;
    uint16_t enrollIndex;
    uint32_t code;
}T_IO_MSG_ENROLL, *PT_IO_MSG_ENROLL;
...
typedef struct
{
    ...
    uint16_t deleteIdx;
    uint32_t code;
}T_IO_MSG_DELETE, *PT_IO_MSG_DELETE;
