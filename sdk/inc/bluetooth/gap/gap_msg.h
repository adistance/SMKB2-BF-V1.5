...
typedef struct
{
    uint8_t gap_init_state: 1;      //!< @ref GAP_INIT_STATE    GAP初始化状态
    uint8_t gap_adv_sub_state: 1;   //!< @ref GAP_ADV_SUB_STATE GAP advertising状态
    uint8_t gap_adv_state: 2;       //!< @ref GAP_ADV_STATE     GAP adv 子状态，仅实用gap_adv_state为GAP_ADV_STATE_IDLE的情况。
    uint8_t gap_scan_state: 2;      //!< @ref GAP_SCAN_STATE    GAP Scan状态
    uint8_t gap_conn_state: 2;      //!< @ref GAP_CONN_STATE    GAP Connection状态
} T_GAP_DEV_STATE;
