...
typedef struct
{
    ...
    uint16_t image_id; //!< indicate different image ->T_IMG_ID
...
typedef struct
{
    T_IMG_CTRL_HEADER_FORMAT ctrl_header;       //IMAGE Header 的控制信息
    uint8_t uuid[16];                           
    uint32_t exe_base;
    uint32_t load_base;
    uint32_t load_len;
    ...
    uint8_t dec_key[16];
    uint8_t rsvd1[28];
    T_VERSION_FORMAT git_ver;                   //版本管理的信息字段
...
typedef struct
{
    T_IMG_CTRL_HEADER_FORMAT ctrl_header;           //OTA Header 的控制信息
    uint8_t uuid[16];
    uint32_t exe_base;
    uint32_t load_base;
    uint8_t rsvd0[8];
    ...
    uint8_t sha256[32];
    uint32_t ver_val;
    uint32_t secure_boot_addr;                      //boot image start address
