...
#define SIMPLE_BLE_SERVICE_CHAR_V3_NOTIFY_INDEX         0x04//0x07
...
#if 0
#define SIMPLE_BLE_PRINT_INFO0   APP_PRINT_INFO0
#define SIMPLE_BLE_PRINT_INFO1   APP_PRINT_INFO1
#define SIMPLE_BLE_PRINT_INFO2   APP_PRINT_INFO2
#define SIMPLE_BLE_PRINT_INFO3   APP_PRINT_INFO3
#define SIMPLE_BLE_PRINT_INFO4   APP_PRINT_INFO4
#else
#define SIMPLE_BLE_PRINT_INFO0(...) 
#define SIMPLE_BLE_PRINT_INFO1(...)	
#define SIMPLE_BLE_PRINT_INFO2(...)	
#define SIMPLE_BLE_PRINT_INFO3(...)	
#define SIMPLE_BLE_PRINT_INFO4(...)	
#endif
...
const T_ATTRIB_APPL simple_ble_service_tbl[] =
{
    /* <<Primary Service>>, .. */
    {
        (ATTRIB_FLAG_VALUE_INCL | ATTRIB_FLAG_LE),  /* flags            ATTRIB_FLAG_VALUE_INCL表示attribute value被置于type_value的最后14字节(type_value的前2个字节用于保存UUID)， 且value_len是放入最后14字节区域的字节数目*/         
        {                                           /* type_value       ATTRIB_FLAG_LE：表示primary service允许通过LE链路访问                                                      */
            LO_WORD(GATT_UUID_PRIMARY_SERVICE),     //0x2800
            HI_WORD(GATT_UUID_PRIMARY_SERVICE),
            LO_WORD(0x1901),      /* service UUID */
            HI_WORD(0x1901)       //0xA00A
        },
    ...
    /* <<Characteristic>> demo for write */
    {
        ...
        {                                           /* type_value */
            LO_WORD(GATT_UUID_CHARACTERISTIC),      //0x2803
            HI_WORD(GATT_UUID_CHARACTERISTIC),
            (GATT_CHAR_PROP_WRITE | /*GATT_CHAR_PROP_WRITE_NO_RSP |*/ GATT_CHAR_PROP_READ) /* characteristic properties */
...
#else
        GATT_PERM_WRITE | GATT_PERM_READ            /* permissions */
#endif
    },
    ...
    /* client characteristic configuration */   //要求client是认证的或有加密的连线，才能对characteristic进行读或写操作。
    {
        ATTRIB_FLAG_VALUE_INCL | ATTRIB_FLAG_CCCD_APPL,                 /* flags */
        {                                         /* type_value */
            LO_WORD(GATT_UUID_CHAR_CLIENT_CONFIG),      //0x2902
            HI_WORD(GATT_UUID_CHAR_CLIENT_CONFIG),
            /* NOTE: this value has an instantiation for each client, a write to */
            /* this attribute does not modify this default value:                */
            LO_WORD(GATT_CLIENT_CHAR_CONFIG_DEFAULT), /* client char. config. bit field */
            HI_WORD(GATT_CLIENT_CHAR_CONFIG_DEFAULT)    //0x0000
        },
        2,                                          /* bValueLen */
        NULL,
#if SIMP_SRV_AUTHEN_EN
...
{
    SIMPLE_BLE_PRINT_INFO4("simple_write_post_callback: conn_id %d, service_id %d, attrib_index 0x%x, length %d",
                                            ...
                                            P_FUN_WRITE_IND_POST_PROC *p_write_ind_post_proc)
{
    ...
    *p_write_ind_post_proc = simple_write_post_callback;
    if (SIMPLE_BLE_SERVICE_CHAR_V2_WRITE_INDEX == attrib_index || 2 == attrib_index)
    {
...
bool simp_ble_service_send_v3_notify(uint8_t conn_id, T_SERVER_ID service_id, void *p_value,
                                     uint16_t length)
{
    SIMPLE_BLE_PRINT_INFO0("simp_ble_service_send_v3_notify");
...
bool simp_ble_service_send_v4_indicate(uint8_t conn_id, T_SERVER_ID service_id, void *p_value,
                                       uint16_t length)
{
    SIMPLE_BLE_PRINT_INFO0("simp_ble_service_send_v4_indicate");
                                     ...
                                     uint16_t cccbits)
{
    ...
    SIMPLE_BLE_PRINT_INFO2("simp_ble_service_cccd_update_cb: index = %d, cccbits 0x%x", index, cccbits);
    switch (index)
    {
