...
/*
static char upgrade_flag = 0;
void test_cb()
{
	uint32_t tmp_base = 0x84E000;
	uint32_t s_val = 0;

	flash_auto_read_locked(tmp_base | FLASH_OFFSET_TO_NO_CACHE, &s_val);
	
	APP_PRINT_INFO1("|****** upgrade flash 0x%x *******|", s_val);
	if(s_val == 0x11223344)
	{
		flash_erase_locked(FLASH_ERASE_SECTOR, 0x84E000);
		upgrade_flag = 1;
	}
...
USER_CALL_BACK app_pre_main_cb __attribute__((weak)) = NULL;//test_cb ;
...
#endif	
	if(get_test_mode() == UPGRADE_MODE)
	{
		reset_test_mode();
		//APP_PRINT_INFO0("----start upgrade -----");
		upgrade_main();
		goto START_SCHEDULER;
	}
...
void ram_init(void)
{	
	...
	//flash_split_read((uint32_t)Load$$RAM_DATA_ON$$RO$$Base, (unsigned int)&Image$$RAM_DATA_ON$$RO$$Length, \
	//				(uint8_t *)Image$$RAM_DATA_ON$$RO$$Base);
	//flash_split_read((uint32_t)Load$$RAM_DATA_ON$$RW$$Base, (unsigned int)&Image$$RAM_DATA_ON$$RW$$Length, \
...
void SystemInit(void)
{
	...
	//flash_try_high_speed(FLASH_MODE_2BIT);
