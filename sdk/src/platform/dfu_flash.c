                    ...
                    uint32_t/*void*/ *p_void)
{
    ...
    {
        result = flash_erase_sector(start_addr);
    }
    else
    {
        ...
        if ((offset / FMC_SEC_SECTION_LEN) != ((offset + length) / FMC_SEC_SECTION_LEN)
            && ((offset % FMC_SEC_SECTION_LEN) + length > FMC_SEC_SECTION_LEN))
        {
            /*flash driver will down align 4K*/
            /*only need erase high addr located sector*/
            result = flash_erase_sector((start_addr + length));
        }
    }
    if (result)
    {
...
bool dfu_copy_img(uint16_t image_id, uint32_t dlAddress, uint32_t dlSize)
{
	...
	DFU_PRINT_INFO2("source_base_addr 0x%x, dest_base_addr 0x%x", source_base_addr, dest_base_addr);
    while (remain_size > 0)
    {
