...
static uint32_t nonce_counter[4] = {0};  // nonce_counter used only for CTR mode
...
bool aes128_ctr_encrypt(uint8_t *plaintext, const uint8_t key[16], uint8_t *encrypted,
                        uint32_t *p_nonce_counter, uint32_t data_word_len)
{
    bool ret;
    ...
    if (p_nonce_counter != 0) { memcpy(nonce_counter, p_nonce_counter, 16); }
    uint8_t *iv_tmp = (uint8_t *)nonce_counter;
    ...
    for (int i = 0; i < data_word_len; i += 4)
    {
        ...
        ret = hw_aes_encrypt128((uint32_t *) nonce_counter, (uint32_t *) out_tmp, 4, (uint32_t *)key_tmp,
                                NULL,
                                AES_MODE_ECB);

        swap_buf(out_tmp, in_tmp, 16);
...
bool aes128_ctr_decrypt(uint8_t *input, const uint8_t key[16], uint8_t *output,
                        uint32_t *p_nonce_counter,
                        uint32_t data_word_len)
{
    bool ret;
                        ...
                        uint32_t *p_nonce_counter, uint32_t data_word_len)
{
    ...
    for (int i = 0; i < data_word_len; i += 4)
    {
        ...
        ret = hw_aes_encrypt256((uint32_t *) nonce_counter, (uint32_t *) out_tmp, 4, (uint32_t *)key_tmp,
                                NULL,
                                AES_MODE_ECB);
...
bool aes256_ctr_decrypt(uint8_t *input, const uint8_t key[32], uint8_t *output,
                        uint32_t *p_nonce_counter,
                        uint32_t data_word_len)
{
    bool ret;
